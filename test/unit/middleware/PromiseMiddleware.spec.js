describe('PromiseMiddleware', function () {
  ...
  it('sendAsync - success', () => {
    return this.getResponse('sendAsync').promise().then((response) => {
      expect(response).toEqual(expect.objectContaining(
        { status: 200, text: 'sendAsync success' }
      ));
    });
  });
  ...
  `('renderAsync - success: will render view $testCase', ({ view, viewProps, textExpectation }) => {
    return this.getResponse('renderAsync', { view, viewProps }).promise().then((response) => {
      expect(response).toEqual(expect.objectContaining(
        { status: 200, text: expect.stringContaining(textExpectation) }
      ));
    });
  });
  ...
  `('formatAsync: %s - success', ({ acceptHttpHeader, expectedData }) => {
    return this.getResponse('formatAsync', { acceptHttpHeader }).promise().then((response) => {
      expect(response).toEqual(expect.objectContaining(
        { status: 200, ...expectedData }
      ));
    });
  });
});
